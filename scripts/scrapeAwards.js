...
function normalizeCategory(category) {
  ...
  // Skip non-categories
  if (category.toLowerCase().includes('sponsor') ||
  ...
  if (category.includes('McFarlane') ||
...
async function extractCategories(url) {
  try {
    ...
    $('.category, .award-category, .category-name, [class*="category"], [class*="award"]').each((i, el) => {
      const text = $(el).text().trim();
      const normalized = normalizeCategory(text);
      if (normalized) {
        categories.add(normalized);
...
function determineRank($, element) {
  ...
  try {
    ...
    if (text.includes('finalist') || text.includes('silver') ||
        className.includes('finalist') || className.includes('silver')) {
      rank = 2;
  ...
  try {
    ...
    $('.winner, .finalist, .award-item, .award-winner, .award-entry').each((index, element) => {
      try {
        ...
        const projectSelectors = ['.winner-name', '.project-name', '.entry-name', '.title', 'h3', 'h4', 'strong'];
        let projectName = '';
        ...
        for (const selector of agencySelectors) {
          ...
          if (agencyElement.length) {
            agencyName = agencyElement.text().trim().replace(/^by\s+|^by:/i, '');
            break;
          }
        }
        ...
        console.log(`Found potential winner: Category=${normalizedCategory}, Project=${projectName}, Agency=${agencyName}, Rank=${rank}`);
    ...
    if (winners.length === 0) {
      ...
      $('section, article, div.winner, div.awards, div.content').each((index, element) => {
        ...
        headings.each((i, heading) => {
          ...
          if (categoryText.toLowerCase().includes('sponsor') ||
          ...
          for (const pattern of winnerPatterns) {
            ...
            if (match) {
              ...
              if (pattern.toString().includes('silver') || pattern.toString().includes('2nd')) {
                rank = 2;
              } else if (pattern.toString().includes('bronze') || pattern.toString().includes('3rd')) {
                rank = 3;
              }

              console.log(`Found pattern match: Category=${normalizedCategory}, Project=${projectName}, Agency=${agencyName}, Rank=${rank}`);
        ...
        $(table).find('tr').each((rowIndex, row) => {
          ...
          if (cells.length >= 2) {
            ...
            if (cells.length >= 3) {
              ...
              if (rankText.includes('winner') || rankText.includes('gold') || rankText.includes('1')) {
                rank = 1;
              } else if (rankText.includes('finalist') || rankText.includes('silver') || rankText.includes('2')) {
                rank = 2;
            ...
            if (category && projectName) {
              console.log(`Found table entry: Category=${category}, Project=${projectName}, Agency=${agencyName}, Rank=${rank}`);
...
async function scrapeAllAwards() {
  ...
  const historicalData = historicalWinners.filter(winner => !allWinners.some(
    w => w.year === winner.year && w.category === winner.category && w.project === winner.project
  ));
  ...
  const additionalWinners = fallbackWinners.filter(winner => !allWinners.some(
